...
foreach($file in $DirList) {
    ...
    foreach($line in $contents) {
        ...
        if ($line -match '[^!]\[[\w\s\d]+\]\(([./]*deployment-guides\/[\w\d./?=-]*([\w\d.?=-]+\.[a-z]+))\)' -or $line -match '[^!]\[[\w\s\d]+\]\([./]*resources\/[\w\d./?=-]*(([\w\d.?=-]+\.[a-z]+))\)') {
            # write-output "$($Directory)/$($Matches[1])"
            write-output "$($tmp)/$($section)/$($bundle)/$($resource)/$($Matches[2])"
            Copy-Item -Path "$($Directory)/$($Matches[1])" -Destination "$($tmp)/$($section)/$($bundle)/$($resource)/$($Matches[2])" -Force
        }
